...
function NumInput({
  ...
  return (
    <span className="inline-flex items-center gap-1">
      ...
      />
      {field.unit && (
        <span className="font-mono text-[10px] text-faint">{field.unit}</span>
      )}
    </span>
  );
}
...
function useWindows(market: string) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await Promise.all(
        Object.keys(WINDOWS).map(async (iv) => {
          try {
            const r = await fetch(
              `/api/candles?pair=${encodeURIComponent(market)}&interval=${iv}&limit=500`,
            );
            const j = await r.json();
            if (r.ok && Array.isArray(j.candles) && j.candles.length >= 10)
              out[iv] = j.candles;
          } catch {
            /* window stays absent; the table shows a dash */
          }
        }),
      );
      if (alive) {
...
export function StrategyBuilder() {
  ...
  const { cleanDef, defErr } = useMemo(() => {
    try {
      return {
        cleanDef: sanitizeCustomDef(def as unknown),
        defErr: null as string | null,
      };
    } catch (e: any) {
  ...
  const historic = useMemo(() => {
    ...
    return Object.keys(WINDOWS).map((iv) => {
      ...
      return {
        iv,
        span: WINDOWS[iv],
        sim: candles ? simulateCustom(cleanDef, candles) : null,
      };
    });
  }, [cleanDef, windows]);
  ...
  const setExit = (key: string, v: number) =>
    setDef((d) => ({
      ...d,
      exits: { ...d.exits, [key]: PCT_EXIT_KEYS.has(key) ? v / 100 : v },
    }));
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          value={def.name}
          onChange={(e) =>
            setDef((d) => ({ ...d, name: e.target.value.slice(0, 60) }))
          }
          ...
          <section className="card">
            ...
            <div className="space-y-2 p-3">
              {def.rules.map((r, i) => (
                <RuleCard
                  ...
                  onChange={(nr) =>
                    setDef((d) => ({
                      ...d,
                      rules: d.rules.map((x, j) => (j === i ? nr : x)),
                    }))
                  }
                  onRemove={() =>
                    setDef((d) => ({
              ...
              <div className="relative" ref={addRef}>
                ...
                {addOpen && (
                  <ul
                    className="absolute z-50 mt-1 max-h-72 w-full overflow-auto rounded-md bg-white/5 py-1 shadow-xl shadow-black/40"
                    role="menu"
                  >
                      ...
                      <li key={spec.kind} role="none">
                        <button
                          ...
                          onClick={() => {
                            setDef((d) => ({
                              ...d,
                              rules: [...d.rules, newRule(spec.kind)],
                            }));
                            setAddOpen(false);
                          }}
          ...
          <section className="card">
            ...
            <div className="space-y-1 p-3">
              {EXIT_FIELDS.map((f) => (
                ...
                >
                  <label htmlFor={`ex-${f.key}`} className="text-xs text-muted">
                    {f.label}
                  </label>
                  <NumInput
                    id={`ex-${f.key}`}
                    field={f}
                    value={
                      PCT_EXIT_KEYS.has(f.key)
                        ? Number(
                            (
                              def.exits[f.key as keyof typeof def.exits] * 100
                            ).toFixed(4),
                          )
                        : def.exits[f.key as keyof typeof def.exits]
                    }
              ...
              <p className="pt-2 text-[11px] leading-relaxed text-faint">
                The hard stop is mandatory - it caps the worst case. Add a
                take-profit, trail or time-stop so winners and stalled trades
                also have a way out.
              </p>
            </div>
          </section>
          ...
          {cleanDef && (
            <section className="rounded-lg border border-accent/40 bg-panel p-4">
              ...
              <p className="mt-2 text-xs leading-relaxed text-dim">
                <span className="font-medium text-gain">BUY</span>{" "}
                {marketLabel(market)} when{" "}
                ...
                . <span className="font-medium text-loss">EXIT</span> via{" "}
                {describeExits(cleanDef.exits)}.
              </p>
            </section>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <section className="overflow-hidden card">
            <div className="flex items-center justify-between px-4 py-3">
              <h2 className="eyebrow">
                Historic analysis - every window, net of fees
              </h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left font-mono text-[11px]">
                <thead className="bg-inset text-faint">
                  <tr>
                    <th className="px-3 py-1.5 font-medium uppercase tracking-wider">
                      Window
                    </th>
                    <th className="px-3 py-1.5 text-right font-medium uppercase tracking-wider">
                      Entries
                    </th>
                    <th className="px-3 py-1.5 text-right font-medium uppercase tracking-wider">
                      Win rate
                ...
                <tbody className="[&>tr:nth-child(odd)]:bg-white/[0.015]">
                  {historic.map(({ iv, span, sim }) => (
                    <tr key={iv}>
                      ...
                      {sim ? (
                        <>
                          <td className="px-3 py-1.5 text-right tnum text-dim">
                            {sim.trades.length}
                          </td>
                          <td className="px-3 py-1.5 text-right tnum text-dim">
                            {sim.winRate === null
                              ? "-"
                              : `${sim.winRate.toFixed(0)}%`}
                          </td>
                          <td
                            className={`px-3 py-1.5 text-right tnum ${sim.closed === 0 ? "text-muted" : sim.totalNetPct >= 0 ? "text-gain" : "text-loss"}`}
                          >
                            {sim.closed ? pct(sim.totalNetPct) : "-"}
                          </td>
                          <td
                            className={`px-3 py-1.5 text-right tnum ${sim.buyHoldPct >= 0 ? "text-gain" : "text-loss"}`}
                          >
                            {pct(sim.buyHoldPct)}
                          </td>
                          <td className="px-3 py-1.5 text-right tnum text-muted">
                            {sim.exposurePct.toFixed(0)}%
                          </td>
                        </>
                      ) : (
                        <td colSpan={5} className="px-3 py-1.5 text-faint">
                          {windowsLoading ? "loading…" : "no data"}
                        </td>
                      )}
                    </tr>
                  ))}
          ...
          <section className="overflow-hidden card">
            <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
              <h2 className="eyebrow">
                Vs the stock templates - same market &amp; window
              </h2>
              <Select
                size="sm"
                ...
                <tbody className="[&>tr:nth-child(odd)]:bg-white/[0.015]">
                  {compare.map((row) => (
                    <tr key={row.name} className={row.you ? "bg-accent/5" : ""}>
                      <td className="px-3 py-1.5 text-dim">
                        {row.you && (
                          <span className="mr-1.5 rounded-sm bg-accent/20 px-1 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-accent">
                            you
                          </span>
                        )}
                        {row.name}
                      ...
                      <td className="px-3 py-1.5 text-right tnum text-dim">
                        {row.sim.winRate === null
                          ? "-"
                          : `${row.sim.winRate.toFixed(0)}%`}
                      </td>
                      <td
                        className={`px-3 py-1.5 text-right tnum ${row.sim.closed === 0 ? "text-muted" : row.sim.totalNetPct >= 0 ? "text-gain" : "text-loss"}`}
                      >
          ...
          <p className="text-[11px] leading-relaxed text-faint">
            All numbers are simulated on live Binance candles, fills at bar
            close, long-only, net of ~{FRICTION_PCT}% round-trip friction (fee +
            GST + TDS). A strategy that only wins on one window is probably fit
            to noise - look for consistency. Past performance never guarantees
