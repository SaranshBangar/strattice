"use client";
// Strategy preview: replays a template over real Binance candles and draws every
...
export function StrategyPreview({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(
          `/api/candles?pair=${encodeURIComponent(pair)}&interval=${interval}&limit=${LIMIT}`,
        );
  ...
  const warmup = Math.min(
    ...
    custom
      ...
      : template
        ? Math.max(
            ...Object.values(mergedParams(template, params)).filter(
  ...
  for (const c of candles) {
    loRaw = Math.min(loRaw, c.l);
    hiRaw = Math.max(hiRaw, c.h);
  }
  for (const s of lines)
    for (const v of s.points)
      if (v !== null) {
        loRaw = Math.min(loRaw, v);
        hiRaw = Math.max(hiRaw, v);
      }
  if (!isFinite(loRaw)) {
    loRaw = 0;
    hiRaw = 1;
  ...
  const xTicks =
    xk <= 1
      ...
      : Array.from({ length: xk }, (_, j) =>
          tsShort(candles[Math.round((j * (n - 1)) / (xk - 1))].t, interval),
        );
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-baseline gap-3">
          <span className="font-mono text-sm font-semibold text-fg">
            {pair.replace(/^I-/, "").replace("_", "/")}
          </span>
          {status === "ok" && (
            <span className="font-mono text-sm tnum text-dim">
              ${fmt(last)}
            </span>
          )}
      ...
      {status === "ok" && sim && (
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
          <Stat
            ...
            hint="Simulated entries in this window"
          />
          <Stat
            label="Win rate"
      ...
      {status === "error" ? (
        <div className="grid h-[300px] place-items-center rounded-md border border-dashed border-line text-sm text-muted">
          Could not load candles for {pair}. Check the market id (e.g.
          I-BTC_INR).
        </div>
      ) : status === "loading" ? (
        ...
        <div>
          <div className="grid" style={{ gridTemplateColumns: "4.5rem 1fr" }}>
            ...
            >
              <svg
                ...
                role="img"
                aria-label={`${pair} candles with simulated entries and exits`}
              >
                {yTicks.map((_, i) => (
                  <line
                    key={i}
              ...
              {trades.map((t, i) => (
                <div key={i} className="pointer-events-none">
                  <span
                    ...
                    style={{
                      left: `${xPct(t.entryIdx)}%`,
                      top: `calc(${yPct(candles[t.entryIdx].l)}% + 4px)`,
                    }}
                  ...
                  {t.exitIdx !== null && (
                    <span
                      ...
                      style={{
                        left: `${xPct(t.exitIdx)}%`,
                        top: `calc(${yPct(candles[t.exitIdx].h)}% - 4px)`,
                      }}
              ...
              {warmup > 8 && warmup < n && (
                <span
                  className="pointer-events-none absolute bottom-1 font-mono text-[9px] uppercase tracking-wider text-faint"
                  style={{ left: 4 }}
                >
              ...
              {hc && (
                <div className="pointer-events-none absolute left-2 top-2 z-10 rounded-md bg-panel px-2.5 py-1.5 font-mono text-[11px] shadow-xl">
                  ...
                  <div className="mt-0.5 tnum text-dim">
                    O {fmt(hc.o)} · H {fmt(hc.h)} · L {fmt(hc.l)} · C{" "}
                    <span className="text-fg">{fmt(hc.c)}</span>
                  </div>
                  {hoverEvent && (
                    <div
                      className={`mt-1 pt-1 ${hoverEvent.kind === "entry" ? "text-gain" : "text-loss"}`}
                    >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 font-mono text-[11px] text-muted">
            <span>
              <span className="text-gain">▲</span> entry
            </span>
            <span>
              <span className="text-loss">▼</span> exit
            </span>
            {lines.map((s) => (
              <span key={s.name} className="inline-flex items-center gap-1.5">
                <span
                  ...
                  style={{
                    borderColor: s.role === "primary" ? C.accent : C.faint,
                    borderTopStyle: s.role === "primary" ? "solid" : "dashed",
                  }}
                />
                {s.name}
              </span>
      ...
      {status === "ok" &&
        (trades.length ? (
                  ...
                  {[...trades].reverse().map((t, i) => (
                    <tr key={i}>
                      <td className="px-3 py-1.5 text-dim">
                        ...
                        {tsShort(candles[t.entryIdx].t, interval)} · $
                        {fmt(t.entryPrice)}
                      </td>
                      <td className="px-3 py-1.5 text-dim">
                        {t.exitIdx !== null ? (
                          <>
                            ...
                            {tsShort(candles[t.exitIdx].t, interval)} · $
                            {fmt(t.exitPrice)}
                          </>
                        ) : (
                      ...
                      <td className="px-3 py-1.5 text-right tnum text-muted">
                        {(t.exitIdx ?? n - 1) - t.entryIdx} bars
                      </td>
                      <td className="px-3 py-1.5 text-muted">
                        {EXIT_REASON_LABEL[t.reason]}
                      </td>
                      <td
                        className={`px-3 py-1.5 text-right tnum ${t.netPct >= 0 ? "text-gain" : "text-loss"}`}
                      >
                        {pct(t.netPct)}
                      </td>
                    </tr>
          ...
          <p className="rounded-md border border-dashed border-line px-3 py-2.5 text-xs text-muted">
            No entries in this window - the stock parameters are deliberately
            picky (regime gate, confirmation, cost gate). Try a longer candle
            interval or another market.
          </p>
        ))}
