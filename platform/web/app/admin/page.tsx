...
export default async function AdminPage({
  ...
  const [stats, list] = await Promise.all([q.adminStats(), q.listUsersAdmin(search, page, 20)]);

  const mrr = Object.entries(stats.byTier).reduce(
    (sum, [tier, n]) => sum + (TIERS[tier as keyof typeof TIERS]?.priceInr ?? 0) * n, 0);
  const totalPages = Math.max(1, Math.ceil(list.total / list.pageSize));
  const tierLine = Object.entries(stats.byTier)
    .sort((a, b) => (TIERS[b[0] as keyof typeof TIERS]?.priceInr ?? 0) - (TIERS[a[0] as keyof typeof TIERS]?.priceInr ?? 0))
    .map(([t, n]) => `${t} ${n}`).join(" · ") || "none";

  const pageHref = (p: number) => `/admin?${new URLSearchParams({ ...(search ? { q: search } : {}), page: String(p) })}`;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 font-mono text-xs text-faint">Owner console · {user.email}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Users" value={String(stats.users)} sub={`${list.total} shown`} />
        <StatCard label="Paying" value={String(stats.paying)} sub={`${inr(mrr)} / mo est.`} tone="good" />
        <StatCard label="Active bots" value={String(stats.activeBots)} sub={`${stats.liveBots} live`} tone={stats.liveBots > 0 ? "warn" : "default"} />
      ...
      <section className="rounded-lg border border-line bg-panel">
        <div className="flex flex-col gap-3 border-b border-line p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <form method="GET" className="flex items-center gap-2">
            ...
            {search && (
              <Link href="/admin" className="rounded-md px-2 py-1.5 text-xs text-muted transition-colors hover:text-fg">Clear</Link>
            )}
          </form>
        </div>

        {list.rows.length === 0 ? (
          <div className="px-4 py-10 text-center text-sm text-muted">No users match “{search}”.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {list.rows.map((u) => <AdminUserRow key={u.id} u={u} />)}
              </tbody>
            </table>
          </div>
        )}
      ...
      <p className="font-mono text-[11px] text-faint">
        Plan changes grant a 30-day comp window and take effect on the user's next dashboard load. Bot stops apply within one supervisor poll.
      </p>
    </div>
  );
}
