...
const pyTiers = JSON.parse(execFileSync("python", ["-c", PY], { encoding: "utf8" }));
...
for (const [name, p] of Object.entries(pyTiers)) {
  ...
  if (!eq) { console.error(`MISMATCH ${name}:`, { ts: t, py: p }); problems++; }
}
if (Object.keys(TIERS).length !== Object.keys(pyTiers).length) { console.error("tier count differs"); problems++; }
