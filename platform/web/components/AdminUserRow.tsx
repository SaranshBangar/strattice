    ...
    <tr className="align-middle odd:bg-white/[0.015] hover:bg-inset/40">
      ...
      <td className="px-4 py-2.5">
        <div className="flex items-center gap-2">
          <Select size="sm" ariaLabel={`Plan for ${u.email}`} value={tier} onChange={apply} options={TIER_OPTS} className="w-24" />
          {paid && <span className="whitespace-nowrap font-mono text-[10px] text-faint">{daysLeft(u.period_end)}</span>}
        </div>
      </td>
