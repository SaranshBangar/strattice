...
export function TradesTable({ trades }: { trades: Trade[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    let rows = trades.filter((t) => {
      ...
      if (
        q &&
        !`${t.market} ${strategyLabel(t.strategy)} ${t.side} ${t.status}`
          .toLowerCase()
          .includes(q)
      )
        return false;
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select
            ...
            options={[
              ...
              ...strategies.map((s) => ({ value: s, label: strategyLabel(s) })),
            ]}
          />
          <Select
            size="sm"
        ...
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr>
                  ...
                  <th className="whitespace-nowrap px-4 py-2.5 text-left font-mono text-[11px] font-medium uppercase tracking-[0.1em] text-faint">
                    Strategy
                  </th>
                  <Th
                ...
                {pageRows.map((t, i) => {
                  ...
                  return (
                    <tr
                      key={i}
                      className={[
                        "odd:bg-white/[0.015] hover:bg-inset/60",
                        executed ? "" : "opacity-60",
                      ].join(" ")}
                    >
                      <td className="whitespace-nowrap px-4 py-2.5 font-mono tnum text-muted">
                        {fmtTs(t.ts)}
                      </td>
                      <td className="whitespace-nowrap px-4 py-2.5 font-mono font-medium text-fg">
                        {strategyLabel(t.strategy)}
                        {t.dry_run ? (
                          <span className="ml-1.5 rounded-sm bg-inset px-1 py-0.5 text-[9px] uppercase tracking-wide text-faint">
                            dry
                          </span>
                        ) : null}
                        {!executed && (
                          <span className="ml-1.5 rounded-sm bg-loss/10 px-1 py-0.5 text-[9px] uppercase tracking-wide text-loss">
                            {t.status}
                          </span>
                        )}
                      </td>
                      ...
                      <td
                        className={[
                          "whitespace-nowrap px-4 py-2.5 font-mono",
                          buy ? "text-gain" : "text-loss",
                        ].join(" ")}
                      >
                        {t.side?.toUpperCase()}
                      </td>
                      <td className="whitespace-nowrap px-4 py-2.5 text-right font-mono tnum text-dim">
                        {fmt(t.qty)}
                      </td>
                      <td className="whitespace-nowrap px-4 py-2.5 text-right font-mono tnum text-dim">
                        {fmt(t.price)}
                      ...
                      <td
                        className={[
                          "whitespace-nowrap px-4 py-2.5 text-right font-mono tnum",
                          p < 0
                            ? "text-loss"
                            : p > 0
                              ? "text-gain"
          ...
          <div className="flex flex-col items-center justify-between gap-3 px-4 py-3 sm:flex-row">
            ...
            <div className="flex items-center gap-1">
              <PageBtn disabled={clampedPage <= 1} onClick={() => setPage(1)}>
                «
              </PageBtn>
              <PageBtn
              ...
              <span className="px-2 font-mono text-xs tabular-nums text-dim">
...
function Th({
  ...
  return (
    <th
      className={[
        "whitespace-nowrap px-4 py-2.5 font-mono text-[11px] font-medium uppercase tracking-[0.1em]",
    ...
    >
      <button
        ...
        onClick={onClick}
        className={[
          "inline-flex items-center gap-1 transition-colors hover:text-dim focus-visible:outline-none",
